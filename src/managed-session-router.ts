...
export class ManagedSessionRouter{
 ...
 async prompt(conversationId:string,profile:AgentProfile,message:string):Promise<{sessionId:string,response:string}>{
  ...
  if(entry.timer)clearTimeout(entry.timer);entry.lastUsed=Date.now();await this.acquire(profile);try{const response=await collect(entry.session,message);this.expireLater(conversationId,entry);return{sessionId:entry.session.sessionId,response};}finally{this.release(profile);}
 }
...
async function collect(session:AgentSession,message:string):Promise<string>{let text="";const off=session.subscribe(event=>{if(event.type==="message_update"&&event.assistantMessageEvent.type==="text_delta")text+=event.assistantMessageEvent.delta;if(event.type==="message_end"&&event.message.role==="assistant"){const final=event.message.content.filter(p=>p.type==="text").map(p=>p.text).join("").trim();if(final)text=final;}});try{await session.prompt(message,{source:"extension"});return text.trim()||"Pi completed without a text response.";}finally{off();}}
